...
function areBrokenSets(equippedSetTypes, currentlyAddedSetType) {
	for (var setName in equippedSetTypes) {
		...
		if( cnt > 0 && cnt != allSets[setName][0] ) {
			return true;
		}
	}
...
function optimize(gridRunes, gridMons, focusSelected) {
	...
	var calculateAndDisplay = function() {
		...
		if(allRunePermutations.length > 0) {
			...
			var step = 20000;
			/*if(allRunePermutations.length >400000)
				step = 40000;*/
			target_optimize_calls_count = Math.ceil(allRunePermutations.length / step);
			var j = 0;
			while( j*step < allRunePermutations.length) {
				$.ajax({
					...
					success: function(data){
						...
						if(optimize_calls_count == target_optimize_calls_count) {
	...
	if(requestedSetNumber == 0 || allOneSet) {
		...
		var getPermutationsLoop1 = function() {
						...
						for(; i3<setsForAllSlots[3].length; i3++) {
							for(; i4<setsForAllSlots[4].length; i4++) {
								for(; i5<setsForAllSlots[5].length; i5++) {
									iterations += 1;
									if(iterations % 100 == 0) {
										$("#opt_time").html("Calculating number of permutations: " + allRunePermutations.length + ". Press Abort to stop.");
										asyncProcess = setTimeout(getPermutationsLoop1, 1);
										return;
									}
									...
									equippedSetTypes[ setsForAllSlots[3][i3]["set"] ] += 1;
									equippedSetTypes[ setsForAllSlots[4][i4]["set"] ] += 1;
									if($("#opt_no_broken").is(':checked') && areBrokenSets(equippedSetTypes, setsForAllSlots[5][i5]["set"])){
										continue;
									}
									
									var perm = [];
									perm.push( JSON.parse(JSON.stringify( setsForAllSlots[0][i0].id )) );
									perm.push( JSON.parse(JSON.stringify( setsForAllSlots[1][i1].id )) );
									perm.push( JSON.parse(JSON.stringify( setsForAllSlots[2][i2].id )) );
									perm.push( JSON.parse(JSON.stringify( setsForAllSlots[3][i3].id )) );
									perm.push( JSON.parse(JSON.stringify( setsForAllSlots[4][i4].id )) );
									perm.push( JSON.parse(JSON.stringify( setsForAllSlots[5][i5].id )) );
...
$(document).ready(function(){
	...
	$("#optimize_calls_count").val("0");
	$("#optimize_run").val("0");
	$('#sessionId').val( moment().format('YYYYMMDDHHmmssSSS') + Math.floor((Math.random() * 100) + 1) );
	...
	$('#btn_export').on( 'click', function(e) {
		...
		$('#exportimport').select();
		fireAlert("success", "Optimizer data sucessfully generated. Total imported runes: " +gridRunes.rows().data().length+". Total imported monsters: "+gridMons.rows().data().length+".<br/> Copy the selected text into a save place.");
	});
	...
	$('#grid_opt tbody').on( 'click', 'tr a.previewBuild', function (e) {
		...
		var monster_x = extendMonster(monster, runes);
	...
	$('body').on('click', '.myAlert', function (e) {
		e.preventDefault();
    	$(this).hide();
	});
